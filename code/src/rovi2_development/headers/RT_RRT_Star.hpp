...
class RT_RRT_Star
{
    public:
        ...
        void expand_and_rewire(std::chrono::duration<int64_t> _time);
        rw::math::Q create_random_node();
        void rewire_random_nodes(std::chrono::duration<int64_t> _time) {};
        void rewire_from_tree_root(std::chrono::duration<int64_t> _time) {};

        std::vector<RT_Node> find_next_path(std::chrono::duration<int64_t> rrt_time);
        std::vector<RT_Node> plan_path() {return std::vector<RT_Node>(); };
